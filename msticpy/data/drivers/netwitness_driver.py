...
class NetwitnessDriver(DriverBase):
    ...
    def _exec_async_search(self, query_job, page_size, timeout):
        ...
        progress_bar = tqdm(total=100, desc="Waiting Netwitness job to complete")
        prev_progress = 0
        ...
        while True:
            ...
            if datetime.now() > end_time:
                raise MsticpyDataQueryError(
                    "Timeout waiting for Netwitness query to complete",
                    f"Job completion reported {query_job['doneProgress']}",
                    title="Netwitness query timeout",
                )
            sleep(1)
    ...
    def _retrieve_job_status(query_job, progress_bar, prev_progress):
        ...
        if stats["is_done"] == "1":
            logger.info(status)
            logger.info("Netwitness job completed.")
            return True
        return False
    ...
    def _retrieve_results(query_job, offset, page_size):
        ...
        progress_bar_paginate = tqdm(
            total=result_count, desc="Waiting Netwitness result to retrieve"
        )
